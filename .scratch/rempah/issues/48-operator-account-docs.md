...
- 2026-08-19: `docs/ops.md` §4b diperbarui — email operator kini `operator@mailtrap.io` (seed demo + syarat email reset), plus subseksi **Setup Resend** (SMTP kustom gratis: host `smtp.resend.com`, port `465`, username `resend`, password = API key Resend, langkah isi SMTP Settings di dashboard Supabase). Resend dipilih karena mengirim ke inbox asli (bukan menampung seperti Mailtrap Email Testing).
- 2026-08-19: Email operator final diganti ke `adminrempah@gmail.com` (inbox asli) dan `docs/ops.md` §4b disesuaikan — seed demo serta syarat email reset kini memakai alamat tersebut. SMTP Resend aktif dan email reset terverifikasi sampai ke inbox asli.
